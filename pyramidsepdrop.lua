...
local function createModel(opt)
   ...
   -- CIFAR networks
   local function basicblock(n, addFeatures, stride, deathRate)
      ...
      con1:add(Convolution(n,n-addFeatures,3,3,1,1,1,1))
      con1:add(SBatchNorm(n-addFeatures))
      con1:add(nn.StochasticDrop(deathRate))
      concate:add(con1)
      
      local con2 = nn.Sequential()
      con2:add(Convolution(n,addFeatures,3,3,1,1,1,1))
      con2:add(SBatchNorm(addFeatures))
      con2:add(nn.StochasticDrop(deathRate))
      concate:add(con2)
   ...
   local function layer(block, baseFeatures, addFeatures, blockNo, count, stride)
      ...
      local s = nn.Sequential()
      for i=1,count do
         s:add(block(features+i*addFeatures, addFeatures, i == 1 and stride or 1,
   ...
   iChannels = 16
   addChannel = alpha/n
   print(' | PyramidDrop-' .. depth .. ' CIFAR-10')
